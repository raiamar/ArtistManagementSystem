...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!validateCsrf($_POST['csrf_token'] ?? '')) {
        ...
        $result = UserHandler::create($_POST);
        if ($result['success']) {
        ...
        if ($userId <= 0) {
            ...
            $result = UserHandler::update($userId, $_POST);
            if ($result['success']) {
...
<header class="border-b border-solid border-gray-300 bg-white">
    <div class="flex justify-between items-center mb-4">
        ...
        <button command="show-modal" commandfor="createUserModal" onclick="openCreateModal()" class="bg-blue-600 text-white px-4 py-2 mr-4 rounded-md hover:bg-blue-700 transition text-sm font-medium">+ New User</button>
    </div>
</header>
        ...
        <table class="w-full text-sm text-left text-gray-500">
            ...
            <tbody>
                <?php foreach ($users['data'] as $user): ?>
                    <?php $sn++; ?>
                    <?php $userData = $user + ['dob_formatted' => date('Y-m-d', strtotime($user['dob']))] ?>
                    <tr class="bg-white border-b hover:bg-gray-50" data-user="<?= h(json_encode($userData)) ?>">
...
<script>
    ...
    function openCreateModal() {
        ...
        document.querySelectorAll('#createUserModal .text-red-600').forEach(function(el) {
            el.remove();
        });
